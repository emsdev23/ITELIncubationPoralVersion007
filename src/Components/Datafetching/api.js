...
api.interceptors.response.use(
  ...
  (error) => {
    // First, try to decrypt error messages from the server
...
// // --- Request Interceptor ---
// // This interceptor now only handles adding headers (encryption removed)
// api.interceptors.request.use(
//   (config) => {
//     // HEADER LOGIC (Your existing code)
//     const token = sessionStorage.getItem("token");
//     const userid = sessionStorage.getItem("userid");
//     if (token) {
//       config.headers["Authorization"] = `Bearer ${token}`;
...
// // --- Response Interceptor ---
// // This interceptor now only handles authentication errors (decryption removed)
// api.interceptors.response.use(
//   // 1. SUCCESS RESPONSE LOGIC (no decryption)
//   (response) => {
//     // Return the response as is, without decryption
//     return response;
//   },
//   // 2. ERROR RESPONSE LOGIC
//   (error) => {
//     // Handle authentication errors (your existing logic)
//     if (
//       error.response?.status === 401 &&
//       (error.response?.data?.message?.includes("Invalid JWT signature") ||
//         error.response?.data?.message?.includes("Token expired") ||
