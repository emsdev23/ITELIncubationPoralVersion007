...
const TrainingMaterialType = forwardRef(({ title = "📚 Training Material Types" }, ref) => {
  ...
  const deleteMaterialType = useCallback(async (typeId) => {
    try {
  ...
  const handleSubmit = useCallback(
    ...
    [validateField, formData, editType, updateMaterialType, createMaterialType, refreshData, showToast]
  );
    ...
    (type) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          setIsDeleting((prev) => ({ ...prev, [type.trainingmattypeid]: true }));
          ...
          try {
            ...
            setIsDeleting((prev) => ({ ...prev, [type.trainingmattypeid]: false }));
          }
        }
      });
    },
  ...
  const columns = useMemo(() => {
    ...
    // Actions column for Admins (Assuming roleid 1 is Admin)
    if (Number(roleid) === 1) {
      baseColumns.push({
        field: "actions",
        headerName: "Actions",
          ...
          return (
            <Box>
              <ActionButton
                ...
                disabled={isSaving || isDeleting[params.row.trainingmattypeid]}
              >
  ...
  const onExportData = useMemo(
    () => (data) => {
      return data.map((item, index) => ({
        ...
        "Created Time": item.trainingmattypecreatedtime ? new Date(item.trainingmattypecreatedtime).toLocaleString() : "",
      }));
    },
    []
  );
    ...
    <Box sx={{ p: 2 }}>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <TextField
                  ...
                  onBlur={(e) => validateField("trainingmattype", e.target.value)}
                  required
                  disabled={isSaving}
      ...
      <StyledBackdrop open={isSaving}>
        ...
        >
          ...
          <Typography sx={{ mt: 2 }}>
            {editType ? "Updating material type..." : "Creating material type..."}
          </Typography>
        </Box>
      </StyledBackdrop>
    </Box>
