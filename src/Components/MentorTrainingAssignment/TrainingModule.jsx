...
const TrainingModule = forwardRef(({ title = "🎓 Training Module", onAssignSuccess }, ref) => {
  ...
  const [selectedTrainingForAssign, setSelectedTrainingForAssign] = useState(null);
  ...
  const handleAssignSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        Swal.fire("Error", err?.message || "Failed to assign training", "error");
      } finally {
        setIsAssigning(false);
      }
    },
    [IP, assignFormData, selectedTrainingForAssign, token, userId, refreshData, showToast, onAssignSuccess],
  );
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        Swal.fire("Error", err?.response?.data?.data || "Operation failed", "error");
        setIsModalOpen(true);
      } finally {
        setIsSaving(false);
      }
        ...
        <form onSubmit={handleAssignSubmit}>
          <DialogContent>
            <Typography variant="body2" sx={{ mb: 2 }}>
              <strong>Module:</strong> {selectedTrainingForAssign?.trainingmodulename}
            </Typography>
          ...
          <DialogActions>
            <Button onClick={() => setIsAssignModalOpen(false)} disabled={isAssigning}>
              Cancel
            </Button>
            <Button
              type="submit"
